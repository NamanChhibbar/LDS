...
class Pipeline(ABC):
  ...
  def __call__(self, texts: str | list[str], **kwargs) -> str | list[str]:
...
class OpenAIPipeline(Pipeline):
  ...
  def generate_summaries(self, texts: list[str], **_) -> list[str]:
    postprocessor = self.postprocessor
    summaries = []
    for text in texts:
      # Create call inputs
      self._create_inputs(text)
  ...
  def _create_inputs(self, text: str) -> int:
    encoder = self.encoder
